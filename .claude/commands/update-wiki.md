...
Arguments: $ARGUMENTS
Format: `<package> <feature-name>`
- `<package>`: either `server` or `utils`
- `<feature-name>`: kebab-case name matching the folder/file in libs (e.g. `rate-limiter`)
...
Do not infer — read the real code to extract accurate types, function signatures, and behaviour.

---

## Step 2 — Create `wiki/<PascalCase-Feature>.md`

Write a new wiki page following the exact style of existing pages (see `wiki/Result-Types.md` as the reference):
