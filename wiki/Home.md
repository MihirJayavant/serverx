...
- **Type-safe routing** — file-per-route Action pattern with full TypeScript
  inference
- **Automatic OpenAPI** — define metadata alongside the handler; no separate
  spec writing
- **Zod validation** — input validation built into `baseHandler` with automatic
  400 responses
- **MCP support** — expose business logic as AI-agent tools sharing the same
  handlers as HTTP
