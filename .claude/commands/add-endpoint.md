...
Arguments: $ARGUMENTS
Expected format: `<feature> <HTTP-METHOD> <brief-description>`
Example: `user GET /search  search users by name`
...
2. Create the handler in `example/<feature>/` if the business logic doesn't already exist.
...
3. Create `example/controllers/<feature>/<verb>-<feature>.action.ts`:
   - Export `path`, `method`, `handler`, `tags`, `description`, `parameters`, `responses`
   - Import the handler from step 2

4. Create `example/controllers/<feature>/<verb>-<feature>.mcp.ts`:
   - Export `name` (snake_case), `description`, `inputSchema` (raw Zod shape), `handler`
   - Import the **same handler** from step 2 — never duplicate logic

5. Register both in the existing router files:
   - Add `import * as <action>` + `router.addAction(<action>)` to `example/controllers/<feature>.ts`
